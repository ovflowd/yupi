...
{
    public class GroupForumDataMessageComposer : Contracts.GroupForumDataMessageComposer
    {
        public override void Compose(ISender session, Group group, UserInfo user)
        {
            ...
            using (var message = Pool.GetMessageBuffer(Id))
            {
                ...
                {
                    if (group.Forum.WhoCanRead == 1) string1 = "not_member";

                    if (group.Forum.WhoCanPost == 1) string2 = "not_member";

                    if (group.Forum.WhoCanThread == 1) string3 = "not_member";
                }

                if (!group.Admins.Contains(user))
                {
                    if (group.Forum.WhoCanRead == 2) string1 = "not_admin";

                    if (group.Forum.WhoCanPost == 2) string2 = "not_admin";

                    if (group.Forum.WhoCanThread == 2) string3 = "not_admin";

                    if (group.Forum.WhoCanMod == 2) string4 = "not_admin";
                }

                if (user != group.Creator)
                {
                    if (group.Forum.WhoCanRead == 3) string1 = "not_owner";

                    if (group.Forum.WhoCanPost == 3) string2 = "not_owner";

                    if (group.Forum.WhoCanThread == 3) string3 = "not_owner";

                    if (group.Forum.WhoCanMod == 3) string4 = "not_owner";
                }
