...
{
    public class GroupMembersMessageComposer : Contracts.GroupMembersMessageComposer
    {
        ...
        // TODO Refactor?
        public override void Compose(ISender session, Group group, uint reqType, UserInfo user, string searchVal = "",
            int page = 0)
        {
            using (var message = Pool.GetMessageBuffer(Id))
            {
                ...
                var groupList = GetGroupUsersByString(group, searchVal, reqType);
                /*
                if(groupList != null)
                {
        ...
        {
            if (string.IsNullOrWhiteSpace(searchVal)) return theGroup.Requests.ToList();
