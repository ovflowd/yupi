...
{
    public class LoadModerationToolMessageComposer : Contracts.LoadModerationToolMessageComposer
    {
        public override void Compose(ISender session, IList<SupportTicket> Tickets, IList<ModerationTemplate> Templates,
            IList<string> UserMessagePresets, IList<string> RoomMessagePresets, UserInfo user)
        {
            using (var message = Pool.GetMessageBuffer(Id))
            {
                ...
                foreach (var current in Tickets) throw new NotImplementedException();

                message.AppendInteger(UserMessagePresets.Count);

                foreach (var text in UserMessagePresets) message.AppendString(text);
