...
{
    public class TalentsTrackMessageComposer : Contracts.TalentsTrackMessageComposer
    {
        public override void Compose(ISender session, TalentType trackType, IList<Talent> talents)
        {
            using (var message = Pool.GetMessageBuffer(Id))
            {
                ...
                foreach (var current in talents)
                {
                    ...
                    var nm = failLevel == -1 ? 1 : 0; // TODO What does this mean?
                    message.AppendInteger(nm);

                    message.AppendInteger(current.Levels.Count);

                    foreach (var child in current.Levels)
                    {
