...
{
    public class SetRoomUserMessageComposer : Contracts.SetRoomUserMessageComposer
    {
        public override void Compose(ISender room, IList<RoomEntity> users)
        {
            using (var message = Pool.GetMessageBuffer(Id))
            {
                ...
                foreach (var user in users) Serialize(message, user);

                room.Send(message);
            }
        }

        public override void Compose(ISender room, RoomEntity user)
        {
            using (var message = Pool.GetMessageBuffer(Id))
            {
        ...
        {
            if (entity is UserEntity) Serialize(messageBuffer, (UserEntity) entity);
            else if (entity is PetEntity) Serialize(messageBuffer, (PetEntity) entity);
            else if (entity is BotEntity) Serialize(messageBuffer, (BotEntity) entity);
        }

        private void Serialize(ServerMessage messageBuffer, UserEntity user)
        {
            var group = user.UserInfo.FavouriteGroup;
