...
namespace Yupi.Model
{
    public class ValidateModel
    {
        ...
        {
            foreach (var model in RoomModel.GetAll())
            {
                var rows = model.Heightmap.Split('\r');
        ...
        {
            var doorRow = rows[(int) model.Door.Y].ToCharArray();

            doorRow[(int) model.Door.X] = rows[(int) model.Door.Y][(int) model.Door.X + 1];

            for (var i = 0; i < rows.Length; i++)
                if (i == model.Door.X) Console.WriteLine(doorRow);
                else Console.WriteLine(rows[i]);
        }
    }
}