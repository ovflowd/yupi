            ...
            uint theGroup)
        {
            ...
            foreach (Item baseItem in item.Items.Keys)
            {
                ...
                if (isGift)
                {
                    ...
                    if (row == null)
                    {
                        session.GetMessageHandler()
                            .GetResponse()
                            .Init(PacketLibraryManager.OutgoingRequest("GiftErrorMessageComposer"));
                ...
                if (isGift)
                {
                    ...
                    using (IQueryAdapter queryReactor = Yupi.GetDatabaseManager().GetQueryReactor())
                    {
                        queryReactor.SetQuery("INSERT INTO items_rooms (item_name,user_id) VALUES (" + itemBySprite.Name +
                                              ", " + toUserId + ")");
                ...
                session.GetMessageHandler()
                    .GetResponse()
                    .Init(PacketLibraryManager.OutgoingRequest("NewInventoryObjectMessageComposer"));
