...
namespace Yupi.Emulator.Game.Users.Inventory.Components
{
    ...
    /// </summary>
     public class AvatarEffectComponent
    {
        /// <summary>
     ...
     public AvatarEffectComponent(uint userId, GameClient client, UserData data)
        {
            _userId = userId;
            _session = client;
     ...
     public void AddNewEffect(int effectId, int duration, short type)
        {
            using (IQueryAdapter queryReactor = Yupi.GetDatabaseManager().GetQueryReactor())
            ...
            GetClient()
                .GetMessageHandler()
                .GetResponse()
                .Init(PacketLibraryManager.OutgoingHandler("AddEffectToInventoryMessageComposer"));
     ...
     public bool HasEffect(int effectId) => effectId < 1 || _effects.Any(x => x.EffectId == effectId);
     ...
     public void StopEffect(int effectId)
        {
            ...
            GetClient()
                .GetMessageHandler()
                .GetResponse()
                .Init(PacketLibraryManager.OutgoingHandler("StopAvatarEffectMessageComposer"));
     ...
     public GameClient GetClient() => _session;
