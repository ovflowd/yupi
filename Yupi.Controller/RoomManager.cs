...
namespace Yupi.Controller
{
    ...
    {
        private readonly List<Room> _loadedRooms;
        ...
        {
            var data = RoomRepository.FindBy(roomId);
        ...
        public void KickAll(Room room)
        {
            room.EachUser(
                session =>
                {
                    if (session.Info.HasPermission("ignore_room_kick")) return;
